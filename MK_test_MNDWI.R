...
# Load single-band raster images.
# IMPORTANT ! -> Images need to be load into the stack correctly in chronological order !

                               MNDWIStack <- list.files            (
                                             path = "E:/DELL/Riparian_Vegetation_Change/Strati cartografici output/2. MNDWI_Max",
                                             pattern = paste("", ".*.tif$", sep = ""),
                                             all.files = FALSE,
                                             full.names = TRUE,
                                             recursive = TRUE      )
                               MNDWIStack
                               s <- lapply(MNDWIStack, raster)
                               s

# The 'brick' function takes a list of raster objects and combines them into one brick object. 
# A brick is a multi-band raster object that allows you to work with all the layers at once.
  
                                bricked_files <- brick(s)
                                gc()                                                     # Free up memory to handle large raster
                                names(bricked_files)
                                mylayers<- paste0('MNDWI_max.',seq(1,25,1));mylayers
                                bricked_files<-bricked_files[[mylayers]]                 # Rename brick with one step sequence
...
# Set year range for analysis (number of years have to be the same as number of bands)
                                ...
                                names(bricked_files)<-paste0('MNDWI_max_',years)
                                names(bricked_files)

# Remove water pixel(oprional)
...
# Write to Results folder
                                 results<- 'E:/DELL/Riparian_Vegetation_Change/Strati cartografici output/2. MNDWI_Max/Results'
...
#############################################
## P-Value Masking with Significant Tau   ##
