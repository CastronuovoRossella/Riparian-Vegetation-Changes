...
Slope <- rast("E:/DELL/Riparian_Vegetation_Change/Layers/2_MNDWI_Max/Results/ts_slopew_clipped.tif")
Tau <- rast("E:/DELL/Riparian_Vegetation_Change/Layers/2_MNDWI_Max/Results/tau_mask005w_clipped.tif")
...
Tau_classify <- function(Tau) {
                                tau_classified <- classify(Tau, rbind(c(-Inf, 0, -1), c(0, Inf, 1)))
...
final_table <- merge(Slope_area_tibble, 
                     ...
                     by.y = "ts_slopew_clipped", 
                     all = TRUE)



# Classify Tau values (positive/negative)
final_table$TAU_CLASS <- ifelse(final_table$tau_mask005w_clipped > 0, "Positive", 
                                ifelse(final_table$tau_mask005w_clipped < 0, "Negative", "NA"))
...
output_path_csv <- "E:/DELL/Riparian_Vegetation_Change/Layers/3_MK_Senn_Tests/MNDWI_surfaces_by_slope.csv"
